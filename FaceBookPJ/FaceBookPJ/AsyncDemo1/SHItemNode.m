...
#import "SHPlaceholderNetworkImageNode.h"
...
@interface SHItemNode ()<ASNetworkImageNodeDelegate>

@property (nonatomic, strong) SHItemViewModel *viewModel;

@property (nonatomic, strong) SHPlaceholderNetworkImageNode *dealImageView;
...
@implementation SHItemNode
+ (BOOL)isRTL {
    return [UIApplication sharedApplication].userInterfaceLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft;
}

-(instancetype)initWithViewModel:(SHItemViewModel *)viewModel{
    if (self = [super init]) {
...
- (void)setup {
    self.dealImageView = [[SHPlaceholderNetworkImageNode alloc] init];
    self.dealImageView.delegate = self;
    self.dealImageView.placeholderEnabled = YES;
    self.dealImageView.placeholderImageOverride = [SHItemStyles placeholderImage];
    self.dealImageView.defaultImage = [SHItemStyles placeholderImage];
...
- (void)updateLabels {
    ...
    if (self.viewModel.titleText) {
        self.titleLabel.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.titleText attributes:[SHItemStyles titleStyle]];
    }
    if (self.viewModel.firstInfoText) {
        self.firstInfoLabel.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.firstInfoText attributes:[SHItemStyles subtitleStyle]];
    }
    
    if (self.viewModel.secondInfoText) {
        self.secondInfoLabel.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.secondInfoText attributes:[SHItemStyles secondInfoStyle]];
    }
    if (self.viewModel.originalPriceText) {
        self.originalPriceLabel.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.originalPriceText attributes:[SHItemStyles originalPriceStyle]];
    }
    if (self.viewModel.finalPriceText) {
        self.finalPriceLabel.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.finalPriceText attributes:[SHItemStyles finalPriceStyle]];
    }
    if (self.viewModel.distanceLabelText) {
        NSString *format = [SHItemNode isRTL] ? @"%@ •" : @"• %@";
        NSString *distanceText = [NSString stringWithFormat:format, self.viewModel.distanceLabelText];
        
        self.distanceLabel.attributedText = [[NSAttributedString alloc] initWithString:distanceText attributes:[SHItemStyles distanceStyle]];
    }
    ...
    if (isSoldOut) {
        ...
        self.soldOutLabelFlat.attributedText = [[NSAttributedString alloc] initWithString:soldOutText attributes:[SHItemStyles soldOutStyle]];
    }
    ...
    if (hasBadge) {
        self.badge.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.badgeText attributes:[SHItemStyles badgeStyle]];
        self.badge.backgroundColor = [SHItemStyles badgeColor];
    }
